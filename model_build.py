...
class CoAttNet(nn.Module): 
    def __init__( self, in_chans=256, out_chans=128, hidden_size=768) -> None:
        super().__init__()
        ...
        self.linear_classifier = nn.Linear(self.hidden_size, 1, bias = False)
        self.linear_survival = nn.Linear(self.hidden_size, out_chans, bias = False)
...
class CTSMamba(nn.Module):
    def __init__(self, ) -> None:
        ...
        self.TSMamba = TSMamba(in_chans=1,
                                out_chans=1,
                                depths=[2,2,2,2],
                                feat_size=[48, 96, 192, 384]) 
        ...
        for param in self.TSMamba.parameters(): 
            param.requires_grad = False 

    def forward(self, x_in1, x_in2): 
        seg1, feats1 = self.TSMamba(x_in1) 
        seg2, feats2 = self.TSMamba(x_in2) 
