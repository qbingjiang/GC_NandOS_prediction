...
parser.add_argument('--save_path_train', type=str, default='./Bresults/pred_train.xlsx',
                    help='Path to save the training predictions Excel file.')
parser.add_argument('--save_path_test', type=str, default='./Bresults/pred_test.xlsx',
                    help='Path to save the testing predictions Excel file.')
parser.add_argument('--metrics_path', type=str, default='./Bresults/metrics_table.csv',
...
# evc1_set1 = r'./BDataset/evc1_dataset_example.xlsx' 
# evc2_set1 = r'./BDataset/evc2_dataset_example.xlsx'  
# X_evc1 = pd.read_excel(evc1_set1 ) 
# X_evc2 = pd.read_excel(evc2_set1 ) 
...
# X_evc1['target_'] = X_evc1['淋巴结转移（0为N0,1为N+）']
# X_evc2['target_'] = X_evc2['淋巴结转移（0为N0,1为N+）']
...
# image_mask_files_times_evc1, label_DrugRest_ptLever_evc1 = prepare_data_v3(X_evc1)
# image_mask_files_times_evc2, label_DrugRest_ptLever_evc2 = prepare_data_v3(X_evc2)
...
# X_train_path_evc1, y_train_evc1 = [ image_mask_files_times_evc1 ], [ label_DrugRest_ptLever_evc1 ] 
# X_train_path_evc2, y_train_evc2 = [ image_mask_files_times_evc2 ], [ label_DrugRest_ptLever_evc2 ] 
...
# test_model_v3(model, X_train_path_evc1, X_evc1, best_cutoff_value, save_path='./BDataset/pred_evc1.xlsx', auc_thres=auc_thres) 
# test_model_v3(model, X_train_path_evc2, X_evc2, best_cutoff_value, save_path='./BDataset/pred_evc2.xlsx', auc_thres=auc_thres) 

### Step 3: Read the result files and calculate the evaluation metrics
df_preds_train = pd.read_excel(save_path_train) 
df_preds_test = pd.read_excel(save_path_test) 
...
# Compile the metrics into a DataFrame and save to a CSV file
# data_metrics = [metrics_DL_1, metrics_DL_2, metrics_DL_3, metrics_DL_4, surv_1, surv_2, surv_4 ]   
data_metrics = [metrics_DL_1, metrics_DL_2, surv_1, surv_2 ]   
df = pd.DataFrame(data_metrics, index=[['DL',]*len(data_metrics), ['train', 'interValid', 'train_surv', 'interValid_surv']] ) 
df.to_csv(metrics_path, encoding='utf-8-sig') 

print("Script completed successfully.")

# python 'train CTSMamba upload2github test example.py' --training_set ./path/to/training_set.xlsx --validation_set ./path/to/validation_set.xlsx --batch_size 16
