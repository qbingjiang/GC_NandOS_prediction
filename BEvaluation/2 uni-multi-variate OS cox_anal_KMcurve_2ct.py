...
def cox_analysis(data, fname='shanxi'): 
    ...
    univariate_results_df.to_csv(f'./BEvaluation/{fname} univariate_cox_analysis.csv')
    ...
    multivariate_summary.to_csv(f'./BEvaluation/{fname} multivariate_cox_analysis.csv')
    ...
    combined_results.to_excel(f'./BEvaluation/{fname} uni-multi-variate_cox_analysis.xlsx')
...
def plot_KMcurves_survRate_at_times(data, fname='shanxi', duration_col = 'time', event_col = 'event', best_cutoff_value = None, 
    ...
    if show_km_fig: 
        ...
        plt.savefig(f'./BEvaluation/KM_curves_{fname}_{duration_col}.png', bbox_inches='tight', dpi=600)
...
df = df.transpose() 
df.to_excel(f'./BEvaluation/KM_curves_OS rate table.xlsx', encoding='utf-8-sig') 
print() 
...
plt.savefig('./BEvaluation/all_KM_curves.svg', bbox_inches='tight') 
