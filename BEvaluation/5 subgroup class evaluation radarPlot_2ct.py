...
def subgroup_evaluation(df_train, df_test1, df_test2, df_test3, categories, categories_num=[3, 4]): 
    ...
    for i in range(len(categories)): 
        ...
        nums_ =  '_'.join(nums) 
        df = df.transpose() 
        df.to_csv(f'./BEvaluation/metrics_table_subgroup_{category}_{nums_}.csv', encoding='utf-8-sig') 
...
for idx, key in enumerate(keys): 
    ...
    filename = f'./BEvaluation/metrics_table_subgroup_{category}_{nums_}.csv' 
    ...
    for sub_colname in nums_sub: 
        ...
        fname=f'./BEvaluation/subgroup radarPlot_{sub_colname}.svg'
...
plt.savefig('./BEvaluation/subgroup radarPlot_all.svg', bbox_inches='tight')
