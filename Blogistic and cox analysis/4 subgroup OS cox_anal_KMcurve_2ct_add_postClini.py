...
def plot_KMcurves_survRate_at_times(data, fname='shanxi', duration_col = 'time', event_col = 'event', best_cutoff_value = None, title=None): 
    ...
    # plt.show()
    plt.savefig(f'./Blogistic and cox analysis/subgroup KM_curves_{fname}_{duration_col}.svg', bbox_inches='tight')
...
plt.savefig('./Blogistic and cox analysis/all_subgroup_KM_curves_add_postClini.svg', bbox_inches='tight')
plt.savefig('./Blogistic and cox analysis/all_subgroup_KM_curves_add_postClini.png', bbox_inches='tight', dpi=400)

df = pd.DataFrame(results, index=index ) 
df.to_excel(f'./Blogistic and cox analysis/subgroup KM_curves_OS rate table_add_postClini.xlsx', encoding='utf-8-sig') 
